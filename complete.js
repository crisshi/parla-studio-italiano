...
function playAudio(name, paragraph = null) {
  ...
  activeAudio = new Audio(`audio/${name}.mp3`);
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    audio.addEventListener('click', () => playAudio(`${audioPrefix}-${index + 1}`));
    ...
    if (existingChinese) {
      ...
      chinese.className = 'exercise-cn hidden-cn'; chinese.textContent = `中文：${translations[index]}`;
      row.append(chinese);
    }
  });
