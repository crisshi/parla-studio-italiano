...
const articleAudio = [...Array(11)].map((_, index) => `decor-dopaminico/article-${String(index + 1).padStart(2, '0')}`);
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    if (audio) {
      ...
      audio.addEventListener('click', () => playAudio(`decor-dopaminico/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
addHiddenChinese('#talk', ['你如何看待多巴胺装饰？', '你认识家里色彩鲜艳的人吗？', '你会如何描述自己家的装饰风格？', '你最喜欢哪些装饰或设计风格？', '如果要翻新或重新装修房子，你会改变什么？'], 'talk');
addHiddenChinese('#exercise', ['我们为客厅选择了鲜艳的颜色。', '我喜欢这个房间的装饰。', '大脑会产生多种化学物质。', '他对这个问题有了更深入的理解。', '我想贴一款花卉墙纸。'], 'exercise');
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`decor-dopaminico/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/decor-dopaminico/article-${String(index).padStart(2, '0')}.mp3`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`decor-dopaminico/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`decor-dopaminico/pattern-${String(index + 1).padStart(2, '0')}`));
