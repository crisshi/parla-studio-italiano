...
wordList.innerHTML = '<div class="word-list-title"><span>PAROLE DELLA LEZIONE</span><span>5 / 5</span></div>';
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    audio.addEventListener('click', () => playAudio(`personalita-invecchiamento/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`personalita-invecchiamento/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/personalita-invecchiamento/article-${String(index).padStart(2, '0')}.mp3?v=1`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`personalita-invecchiamento/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`personalita-invecchiamento/pattern-${String(index + 1).padStart(2, '0')}`));
