...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    audio.addEventListener('click', () => playAudio(`burger-king-sfida/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
// Cross-browser speech state for Chrome and Edge.
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`burger-king-sfida/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/burger-king-sfida/article-${String(index).padStart(2, '0')}.mp3?v=1`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`burger-king-sfida/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`burger-king-sfida/pattern-${String(index + 1).padStart(2, '0')}`));
