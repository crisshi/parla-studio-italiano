...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    audio.addEventListener('click', () => playAudio(`vivere-piu-a-lungo/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`vivere-piu-a-lungo/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/vivere-piu-a-lungo/article-${String(index).padStart(2, '0')}.mp3`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`vivere-piu-a-lungo/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`vivere-piu-a-lungo/pattern-${String(index + 1).padStart(2, '0')}`));
