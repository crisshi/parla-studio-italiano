...
const articleAudio = [...Array(11)].map((_, index) => `parco-tematico-dragon-ball-parigi/article-${String(index + 1).padStart(2, '0')}`);
const articleParagraphMap = [...Array(11)].map((_, index) => index);
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    if (audio) {
      ...
      audio.addEventListener('click', () => playAudio(`parco-tematico-dragon-ball-parigi/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
addHiddenChinese('#talk', ['你如何看待新建《龙珠》主题乐园的计划？', '你上一次去主题乐园是什么时候？', '你所在国家最大的主题乐园有哪些？你去过吗？', '你最喜欢哪种主题乐园游乐设施？', '你体验过的最佳主题乐园项目有哪些？'], 'talk');
addHiddenChinese('#exercise', ['我意识到自己已经看过这部电影。', '该项目获得了批准。', '这家公司将同意资助该乐园。', '一个区域将专门用于这个系列。', '该乐园仍在建设中。'], 'exercise');
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`parco-tematico-dragon-ball-parigi/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/parco-tematico-dragon-ball-parigi/article-${String(index).padStart(2, '0')}.mp3`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`parco-tematico-dragon-ball-parigi/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`parco-tematico-dragon-ball-parigi/pattern-${String(index + 1).padStart(2, '0')}`));
