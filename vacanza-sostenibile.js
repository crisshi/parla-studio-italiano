...
const articleAudio = [...Array(12)].map((_, index) => `vacanza-sostenibile/article-${String(index + 1).padStart(2, '0')}`);
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    if (audio) {
      ...
      audio.addEventListener('click', () => playAudio(`vacanza-sostenibile/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
addHiddenChinese('#talk', ['你如何看待文章中的建议？', '你会采取措施让自己的假期更加可持续吗？', '你旅行时通常会轻装出行吗？', '你住过环保酒店吗？', '旅行时，你会尽量去当地的咖啡馆和餐厅用餐吗？'], 'talk');
addHiddenChinese('#exercise', ['格蕾塔·通贝里是一名气候活动人士。', '这项研究强调了环境风险。', '直飞航班可以减少排放。', '我努力减少自己的碳足迹。', '我们选择了一家更加可持续的酒店。'], 'exercise');
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`vacanza-sostenibile/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/vacanza-sostenibile/article-${String(index).padStart(2, '0')}.mp3`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`vacanza-sostenibile/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`vacanza-sostenibile/pattern-${String(index + 1).padStart(2, '0')}`));
