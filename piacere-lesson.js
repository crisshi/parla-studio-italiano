...
.alpha-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(104px,1fr));gap:9px}.alpha{display:flex;align-items:center;justify-content:space-between;gap:6px;background:#fff;border:1px solid #ddd6ca;padding:10px 12px;box-shadow:4px 4px 0 #e5dbc8}.alpha>div b{display:block;font:600 21px Georgia,serif;color:#4c55a7;line-height:1.1}.alpha>div small{display:block;font:10.5px/1.3 Arial;color:#8c94a1;margin-top:3px}.alpha .audio-orb{width:24px!important;height:24px!important;background-size:11px!important;margin-left:0!important}.alpha.str{border-color:#e5a469;box-shadow:4px 4px 0 #f2e3cd}
...
const list = document.createElement('aside'); list.className = 'word-list'; list.innerHTML = `<div class="word-list-title"><span>PAROLE DELLA LEZIONE</span><span>${rows.length} / ${rows.length}</span></div>`;
