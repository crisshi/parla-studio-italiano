...
const articleAudio = [...Array(12)].map((_, index) => `nomadi-digitali-seoul-tokyo/article-${String(index + 1).padStart(2, '0')}`);
const articleParagraphMap = [...Array(12)].map((_, index) => index);
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    ...
    if (audio) {
      ...
      audio.addEventListener('click', () => playAudio(`nomadi-digitali-seoul-tokyo/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
addHiddenChinese('#talk', ['你觉得自己会喜欢在前十名中的哪座城市工作？', '你认识数字游民吗？他们从事什么工作？', '你认为数字游民生活方式最吸引人的地方是什么？', '你的工作允许远程办公吗？', '你曾经在旅行时工作过吗？'], 'talk');
addHiddenChinese('#exercise', ['我远程工作四年了。', '首尔的下载速度很快。', '这座城市提供许多文化活动。', '我在市中心寻找价格合理的住房。', '马德里获得了不错的分数。'], 'exercise');
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`nomadi-digitali-seoul-tokyo/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/nomadi-digitali-seoul-tokyo/article-${String(index).padStart(2, '0')}.mp3`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`nomadi-digitali-seoul-tokyo/article-${String(index + 1).padStart(2, '0')}`, paragraph));
...
document.querySelectorAll('#patterns article').forEach((card, index) => {
  ...
  button.addEventListener('click', () => playAudio(`nomadi-digitali-seoul-tokyo/pattern-${String(index + 1).padStart(2, '0')}`));
