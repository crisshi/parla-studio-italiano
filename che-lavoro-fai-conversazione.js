...
extraStyle.textContent += '.exercise-card{max-width:none;margin:0 0 24px;padding:24px 28px;background:#fffdf9;border:1px solid #ddd6ca;border-top:3px solid #16243a;box-shadow:7px 7px 0 #e5dbc8}.exercise-card h3{margin:0 0 12px;font:600 23px Georgia,serif}.exercise-card li{margin:9px 0;font:18px/1.55 Georgia,serif}.exercise-card select{margin-left:12px;padding:7px 10px;border:1px solid #c7cdd5;background:#fff;color:#16243a}.match-grid{display:grid;grid-template-columns:minmax(220px,1fr) minmax(180px,1fr);gap:12px;align-items:center}.mini-choice button,.ordering button{margin:5px;padding:7px 11px;border:1px solid #c7cdd5;background:#fff;color:#4c55a7;cursor:pointer}.mini-choice button.selected,.ordering button.selected{background:#f2c86b;color:#16243a}.choice-result,.order-result{display:inline-block;min-width:100px;min-height:24px;border-bottom:1px solid #4c55a7;color:#4c55a7;font-weight:700}.ordering{padding:13px 0;border-bottom:1px solid #e2ddd4}.order-result{display:block;margin-bottom:7px}.jump-dialogue{padding:9px 13px;border:0;background:#16243a;color:#fff;cursor:pointer}select.correct,.order-result.correct,.choice-result.correct{color:#287352;border-color:#287352}select.wrong,.order-result.wrong,.choice-result.wrong{color:#c44d4d;border-color:#c44d4d}@media(max-width:680px){.match-grid{grid-template-columns:1fr}.exercise-card{padding:20px 16px}.exercise-card select{margin-left:0;max-width:100%}}';
...
document.querySelectorAll('.ordering').forEach((group) => {
  ...
  group.querySelectorAll('button').forEach((button) => button.addEventListener('click', () => {
    ...
    const punctuation = button.dataset.punct;
    result.textContent = punctuation ? `${result.textContent.trimEnd()}${punctuation}` : `${result.textContent}${result.textContent ? ' ' : ''}${button.textContent}`;
  }));
});
