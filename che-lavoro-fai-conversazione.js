...
extraStyle.textContent += '.exercise-card{max-width:none;margin:0 0 24px;padding:24px 28px;background:#fffdf9;border:1px solid #ddd6ca;border-top:3px solid #16243a;box-shadow:7px 7px 0 #e5dbc8}.exercise-card h3{margin:0 0 12px;font:600 23px Georgia,serif}.exercise-card li{margin:9px 0;font:18px/1.55 Georgia,serif}.exercise-card select{margin-left:12px;padding:7px 10px;border:1px solid #c7cdd5;background:#fff;color:#16243a}.match-grid{display:grid;grid-template-columns:minmax(220px,1fr) minmax(180px,1fr);gap:12px;align-items:center}.mini-choice button,.ordering button{margin:5px;padding:7px 11px;border:1px solid #c7cdd5;background:#fff;color:#4c55a7;cursor:pointer}.mini-choice button.selected,.ordering button.selected{background:#f2c86b;color:#16243a}.choice-result,.order-result{display:inline-block;min-width:100px;min-height:24px;border-bottom:1px solid #4c55a7;color:#4c55a7;font-weight:700}.ordering{padding:13px 0;border-bottom:1px solid #e2ddd4}.order-controls{margin-top:10px}.order-controls button{border-radius:999px;background:#fffdf9!important;color:#59657a!important;font-size:13px!important}.order-result{display:block;margin-bottom:7px}.jump-dialogue{padding:9px 13px;border:0;background:#16243a;color:#fff;cursor:pointer}select.correct,.order-result.correct,.choice-result.correct{color:#287352;border-color:#287352}select.wrong,.order-result.wrong,.choice-result.wrong{color:#c44d4d;border-color:#c44d4d}@media(max-width:680px){.match-grid{grid-template-columns:1fr}.exercise-card{padding:20px 16px}.exercise-card select{margin-left:0;max-width:100%}}';
...
function playAudio(name, paragraph = null) {
  ...
  activeAudio = new Audio(`audio/${name}.mp3?v=3`);
...
const articleAudio = [...Array(15)].map((_, index) => `che-lavoro-fai-conversazione/article-${String(index + 1).padStart(2, '0')}`);
const articleParagraphMap = [...Array(15)].map((_, index) => index);
...
function addHiddenChinese(sectionId, translations, audioPrefix) {
  ...
  rows.forEach((row, index) => {
    let audio = row.querySelector('[data-audio]');
    if (audio) {
      audio.classList.add('listen-mini');
    } else {
      ...
      audio.addEventListener('click', () => playAudio(`che-lavoro-fai-conversazione/${audioPrefix}-${String(index + 1).padStart(2, '0')}`));
...
addHiddenChinese('#talk', ['你住在哪里？', '你做什么工作？', '你在哪里工作？'], 'talk');
addHiddenChinese('#exercise', ['晚上好，Ling。', '我住在台北。', '我是商店店员。', '我在商店工作。', '你住在哪里？', '你是老师吗？', '你在学校工作吗？'], 'exercise');
...
document.querySelector('#check').addEventListener('click', () => {
  ...
  document.querySelector('#result').textContent = `${score}/${blanks.length} corrette`;
});
...
document.querySelectorAll('.mini-choice').forEach((group) => {
  group.querySelectorAll(':scope > button').forEach((button) => button.addEventListener('click', () => {
    group.querySelectorAll(':scope > button').forEach((item) => item.classList.toggle('selected', item === button));
    group.querySelector('.choice-result').textContent = button.textContent;
  }));
});
document.querySelectorAll('.ordering').forEach((group) => {
  const result = group.querySelector('.order-result');
  const wordButtons = [...group.querySelectorAll('button')], history = [];
  const rebuild = () => {
    let text = '';
    history.forEach((button) => { text = button.dataset.punct ? `${text.trimEnd()}${button.dataset.punct}` : `${text}${text ? ' ' : ''}${button.textContent}`; });
    result.textContent = text || '______'; result.classList.remove('correct', 'wrong');
  };
  wordButtons.forEach((button) => button.addEventListener('click', () => { history.push(button); button.classList.add('selected'); button.disabled = true; rebuild(); }));
  const controls = document.createElement('div'); controls.className = 'order-controls';
  const undo = document.createElement('button'); undo.type = 'button'; undo.textContent = '↶ 撤回一步';
  undo.onclick = () => { const button = history.pop(); if (!button) return; button.disabled = false; button.classList.remove('selected'); rebuild(); };
  const restart = document.createElement('button'); restart.type = 'button'; restart.className = 'order-restart'; restart.textContent = '⟳ 重做本题';
  restart.onclick = () => { history.splice(0); wordButtons.forEach((button) => { button.disabled = false; button.classList.remove('selected'); }); rebuild(); };
  controls.append(undo, restart); group.append(controls);
});
document.querySelector('.jump-dialogue')?.addEventListener('click', () => document.querySelector('nav button[data-tab="read"]')?.click());
const baseCheck = document.querySelector('#check').onclick;
document.querySelector('#check').addEventListener('click', () => {
  document.querySelectorAll('#exercise select[data-answer]').forEach((select) => {
    select.classList.toggle('correct', select.value === select.dataset.answer);
    select.classList.toggle('wrong', select.value !== select.dataset.answer);
  });
  document.querySelectorAll('.mini-choice').forEach((group) => {
    const result = group.querySelector('.choice-result');
    result.classList.toggle('correct', result.textContent === group.dataset.answer);
    result.classList.toggle('wrong', result.textContent !== group.dataset.answer);
  });
  document.querySelectorAll('.ordering').forEach((group) => {
    const result = group.querySelector('.order-result');
    result.classList.toggle('correct', result.textContent === group.dataset.answer);
...
document.querySelector('#redo').addEventListener('click', () => {
  document.querySelectorAll('#exercise select').forEach((select) => { select.value = ''; select.classList.remove('correct', 'wrong'); });
  document.querySelectorAll('.order-restart').forEach((button) => button.click());
  document.querySelectorAll('.choice-result,.order-result').forEach((result) => { result.textContent = '______'; result.classList.remove('correct', 'wrong'); });
  document.querySelectorAll('.mini-choice button,.ordering button').forEach((button) => { button.disabled = false; button.classList.remove('selected'); });
});

// Lessons without prerecorded MP3s use the browser's Italian speech engine.
// Cancelling and rebuilding from charIndex avoids Chromium's unreliable
// speechSynthesis.resume() behavior with long utterances.
let speechSession = null;
let activeSpeechUtterance = null;
let cachedItalianVoice = null;
function refreshItalianVoice() {
  const voices = speechSynthesis.getVoices();
  cachedItalianVoice = voices.find((voice) => /^it[-_](IT|CH)$/i.test(voice.lang))
    || voices.find((voice) => /^it/i.test(voice.lang))
    || voices.find((voice) => voice.default)
    || voices[0]
    || null;
}
refreshItalianVoice();
speechSynthesis.addEventListener?.('voiceschanged', refreshItalianVoice);
function speakItalian(text, paragraph = null, onEnd = null, startAt = 0) {
  if (!('speechSynthesis' in window)) { status.textContent = '当前浏览器不支持语音朗读。'; return; }
  if (speechSession) speechSession.interrupted = true;
  speechSynthesis.cancel();
  clearHighlight();
  const session = { text, paragraph, onEnd, charIndex: startAt, paused: false, interrupted: false };
  ...
  utterance.onboundary = (event) => {
    session.charIndex = startAt + event.charIndex;
    if (!words.length || event.name !== 'word') return;
    words.forEach((word) => word.classList.remove('is-speaking'));
    const progress = session.charIndex / Math.max(1, text.length);
    words[Math.min(words.length - 1, Math.floor(progress * words.length))]?.classList.add('is-speaking');
  };
  utterance.onend = () => {
    if (session.interrupted) return;
    words.forEach((word) => word.classList.remove('is-speaking'));
    speechSession = null; onEnd?.();
  };
  status.textContent = '正在朗读意大利语。';
  window.setTimeout(() => {
    if (speechSession !== session || session.interrupted) return;
    refreshItalianVoice();
    if (cachedItalianVoice) activeSpeechUtterance.voice = cachedItalianVoice;
...
document.querySelectorAll('[data-speech]').forEach((button, index) => {
  ...
  button.addEventListener('click', () => playAudio(`che-lavoro-fai-conversazione/speech-${String(index + 1).padStart(2, '0')}`, button.closest('.reading p')));
});
...
if (!document.querySelector('.reading [data-audio]')) {
  ...
  listen.addEventListener('click', () => {
    ...
    const next = () => {
      ...
      if (activeAudio) { activeAudio.pause(); activeAudio.currentTime = 0; }
      activeAudio = new Audio(`audio/che-lavoro-fai-conversazione/article-${String(index).padStart(2, '0')}.mp3`);
...
readingParagraphs.forEach((paragraph, index) => {
  ...
  button.addEventListener('click', () => playAudio(`che-lavoro-fai-conversazione/article-${String(index + 1).padStart(2, '0')}`, paragraph));
  paragraph.append(' ', button);
});

document.querySelectorAll('#patterns article').forEach((card, index) => {
  const heading = card.querySelector('h3');
  if (!heading || card.querySelector('[data-speech], [data-audio]')) return;
  ...
  button.addEventListener('click', () => playAudio(`che-lavoro-fai-conversazione/pattern-${String(index + 1).padStart(2, '0')}`));
